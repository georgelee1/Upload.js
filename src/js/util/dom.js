...
const MATCHERS = {
  ...
  css(css, ele) {
    ...
    return css.every(c => classes.indexOf(c) >= 0);
  },
};
...
export class Matcher {
  ...
  test(ele) {
    return flat(ele).every(e => {
      let next = e;
      let result = false;
      const everyMatcher = m => m(next);
      while (next && next.parentNode !== next) {
        result = this._matchers.every(everyMatcher);
        if (!result) {
          if (this._bubble) {
...
/**
 * The DOM List is a wrapper around a list of DOM elements that allow easy modification of the
...
export class DOMList {
  ...
  /**
   * Traverses up the document tree to find the parent the first parent of the DOM elements within
   * this DOMList. If matcher is not supplied will return the parent of the first DOM element in
   * this DOMList.
   */
  parent(matcher) {
  ...
  */
  attr(key, val) {
    if (typeof key === 'object') {
      ...
      let result = undefined;
      this.items.some(ele => {
        const attr = ele.getAttribute(key);
  ...
  /**
   * Uses the <ele>.querySelectorAll(..) to find the appropriate nodes from all the DOM elements
   * within this DOMList and returns them in a wrapped DOMList
   */
  find(selector) {
    const found = [];
    this.items.forEach(ele => {
      const result = ele.querySelectorAll(selector);
  ...
  /**
   * Calls each of the passed handlers for each of the DOM elements within this DOMList with the
   * DOM element as the only parameter
   */
  each(...handlers) {
...
/**
 * The purpose of the SimpleDOMParser is to parse an expressive HTML string into DOM elements.
 * The style of the expressive HTML string is simple on purpose with only the element tag name
 * and class names definable. Hierarchy is expressed using parenthesis.
 *
 * The SimpleDOMParser uses a cache to provide a level of performance for repeated calls. By
 * default the size of the cache is limited to 10. DOM elements are memory expensive, increasing
 * the size of the cache should be done with caution. The cache will evict entries based on when
 * they were last accessed, evicting the oldest first.
 *
 * Examples:
 ...
 * result.outerHTML == '<div><div class="child"></div><div class="child sibling"></div>
...
export class SimpleDOMParser {
  ...
  _build(val) {
    ...
    val.split(/(\s*[ \s\(\)]\s*)/).forEach(t => {
      ...
      if (token === ')') {
        const back = hierarchy.pop();
        if (!back) {
          throw new Error('ExpressiveDOMParser: Invalid location for closing parenthesis');
        }
        node = back.node;
        ...
        if (!node) {
          throw new Error('ExpressiveDOMParser: Invalid location for opening parenthesis');
        }
        hierarchy.push({
          node,
          nodes,
    ...
    if (hierarchy.length) {
      throw new Error('ExpressiveDOMParser: Unmatched opening and closing parenthesis');
    }
