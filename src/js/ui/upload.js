...
export class UploadItem extends Item {
  ...
  _done(done, data) {
    ...
    const complete = this._widget._parser.parse(
      this._widget._opts.get('template.done')).appendTo(this._ele);

    const id = data['upload-image-id'] || data.uploadImageId;
